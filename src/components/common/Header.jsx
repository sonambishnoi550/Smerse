...
const Header = () => {
    ...
    return <div className="bg-light-black/50">
        ...
        <div className="container max-w-[1140px] mx-auto px-4 flex items-center justify-between md:py-4 py-[5px]">
            <a href="/"><img src="../assets/images/png/logo.png" alt="logo" className="md:size-[68px] size-10 pointer-events-none" /></a>
            <ul className="flex items-center gap-5 max-lg:hidden">
                {HEADER_LIST.map(function (item, i) {
                    return <li key={i}><a href={item.link} className="font-semibold text-white text-base relative after:absolute after:h-[1px] after:w-0 after:bg-black after:left-0 hover:after:w-full after:bottom-0 after:transition-all after:duration-300">{item.title}</a></li>
                })}
            </ul>
        ...
        <div className={`w-full h-full bg-black transition-all duration-500 left-0 lg:-top-full z-40 fixed flex gap-5 flex-col justify-center items-center ${open ? "top-0 left-0" : "-top-full"
            ...
            {HEADER_LIST.map(function (item, i) {
                return <a key={i} onClick={() => setOpen(false)} href={item.link} className="font-semibold text-base text-white">{item.title}</a>
            })}
